...
| DataFlowKit 规划节点中预览占位 | 6 | 已注册、已有 `config_schema`、已有 handler，但真实副作用、真实调度或真实循环暂未执行；插件节点已先补执行前校验，跳转、条件和循环类节点已输出控制状态表 |
| DataFlowKit 规划节点中未实现 | 1 | 已有方案文档，尚未进入默认注册表和后端 handler |
...
5. `ConditionFlagNode`、跳转类预览控制节点和循环类预览控制节点已完成，后续如需真实分支或真实循环执行，必须先补条件边、循环边或动态调度协议。
```
