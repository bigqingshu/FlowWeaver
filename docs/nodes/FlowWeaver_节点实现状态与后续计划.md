...
| DataFlowKit 规划节点中预览占位 | 7 | 已注册、已有 `config_schema`、已有 handler，但真实副作用、真实调度、真实循环或真实子工作流暂未执行；插件节点已先补执行前校验，跳转、条件、循环和子工作流类节点已输出控制状态表 |
| DataFlowKit 规划节点中未实现 | 0 | 当前 DataFlowKit 初步提取范围内的节点均已有后端注册或预览落点 |
...
当前 DataFlowKit 初步提取范围内暂无完全未注册的节点。后续工作重点转为真实调度、真实子工作流、外部副作用和节点边界测试的分批加固。
...
| 5 | 循环执行起点、循环判断回跳 | 已完成预览版：输出 `loop_plan` / `loop_decision` 控制状态表，不改变 DAG 执行路径 |
| 6 | 节点组与子工作流 | 已完成预览版：输出 `subworkflow_plan` 控制状态表，不创建子 `WorkflowRun` |
...
5. `ConditionFlagNode`、跳转类、循环类和子工作流预览控制节点已完成，后续如需真实分支、真实循环或真实子工作流执行，必须先补条件边、循环边、父子 run 或动态调度协议。
```

低耦合纯表节点已经完成，写入类、外部资源类节点已开始逐步升级为可控真实执行；控制流批次已先落地不改变调度路径的条件结果节点、预览控制计划节点和子工作流预览计划节点。
