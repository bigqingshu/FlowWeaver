...
| DataFlowKit 规划节点中预览占位 | 3 | 已注册、已有 `config_schema`、已有 handler，但真实副作用或真实调度暂未执行；插件节点已先补执行前校验，跳转锚点和无条件跳转已输出控制状态表 |
| DataFlowKit 规划节点中未实现 | 4 | 已有方案文档，尚未进入默认注册表和后端 handler |
