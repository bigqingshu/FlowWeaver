...
| DataFlowKit 规划节点中预览占位 | 2 | 已注册、已有 `config_schema`、已有 handler，但真实副作用或真实调度暂未执行；插件节点已先补执行前校验，跳转锚点已输出控制状态表 |
| DataFlowKit 规划节点中未实现 | 5 | 已有方案文档，尚未进入默认注册表和后端 handler |
...
| 3 | 条件跳转、无条件跳转 | 后置到通用控制信号、条件边或动态调度协议之后 |
| 4 | 循环执行起点、循环判断回跳 | 迭代上下文、最大迭代保护、终止状态 |
| 5 | 节点组与子工作流 | 子图输入输出映射和运行记录层级 |
...
5. `ConditionFlagNode` 第一版已完成，后续控制流节点必须先补统一控制信号或条件边协议。
```

低耦合纯表节点已经完成，写入类、外部资源类节点已开始逐步升级为可控真实执行；控制流批次已先落地不改变调度路径的条件结果节点。
