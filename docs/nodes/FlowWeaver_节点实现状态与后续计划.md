...
| 节点方案 | 主要原因 | 推荐批次 |
|---|---|---|
| 条件判断节点 | 需要和工作流调度结果表达对齐 | 控制流批次 |
| 条件跳转节点 | 会影响 DAG/执行路径，需单独收口执行语义 | 控制流批次 |
| 无条件跳转节点 | 会影响 DAG/执行路径，需单独收口执行语义 | 控制流批次 |
| 跳转锚点节点 | 与跳转节点配套，需要执行路径识别 | 控制流批次 |
...
| 顺序 | 节点 | 交付内容 |
|---:|---|---|
| 1 | 条件判断节点 | 条件结果输出和后续控制流接口 |
| 2 | 条件跳转、无条件跳转、跳转锚点 | 执行路径识别和运行记录表达 |
| 3 | 循环执行起点、循环判断回跳 | 迭代上下文、最大迭代保护、终止状态 |
| 4 | 节点组与子工作流 | 子图输入输出映射和运行记录层级 |

## 下一步最小目标

建议下一次继续外部资源节点收口时，进入 `PluginNode` 的真实发现器和执行 runner 边界：

```text
1. 将当前 plugin_manifest 草稿替换或补充为正式插件清单读取来源。
2. 定义插件 runner 接口，但先只允许受控内置或测试插件接入。
3. 明确 in_process 与 external_process 的资源隔离、超时、取消和输出协议。
4. 继续保留 allow_external_actions 和 enable_execute 双开关。
5. 对外部动作插件保持默认跳过，避免无确认后台执行。
```

低耦合纯表节点已经完成，写入类和外部资源类节点已开始逐步升级为可控真实执行。
