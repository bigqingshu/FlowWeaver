...
| 批次 | 范围 | 当前状态 | 提交/验证 |
|---|---|---|---|
| 1 | 节点运行路径权限检查 | 已完成 | `8666a51`，已移除节点任务权限句柄生成、内置节点执行前权限检查和对应测试。 |
| 2 | 运行期权限/审计存储接口 | 已完成 | `95c353d`，已移除权限审计协议、Store CRUD、数据库模型/API 和迁移清理。 |
| 3 | 桌面端审计日志界面 | 已完成 | `39bd27e`，已移除 AuditEvent DTO/API Client、审计列表、刷新命令和 `LogsAuditPage`，日志页收口为 RuntimeEvent。 |
| 4 | 文档一致性同步 | 本次提交完成 | README、早期规范状态说明、活跃 UI 计划和当前阶段总纲已同步为“主程序权限审计已移除/后置”。 |
