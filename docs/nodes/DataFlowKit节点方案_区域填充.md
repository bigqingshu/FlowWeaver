# 节点方案：FillRangeNode

更新时间：2026-07-05

## 基本信息

DataFlowKit 来源：`core.area_fill` / 区域填充

FlowWeaver 暂定类型名：`FillRangeNode`

节点方向：数据处理 / 区域单元格填充

优先级：P1

当前状态：规划中，代码未实现

## 要解决的问题

对指定字段范围和行范围组成的二维区域进行填充。

用户看到的能力：选择起止字段、起止行、来源值和覆盖策略，运行后得到区域填充后的新表。

不解决的内容：不编辑外部 Excel 文件，不保留表格样式，不操作输入表本身。

## 输入输出

输入：一个 `TableRef`。

输出：一个新的 `TableRef`。

输出行数和字段数通常不变，只改变目标区域内单元格值。

## 配置项草案

| 字段 | 含义 |
|---|---|
| `start_field` / `end_field` | 起止字段 |
| `start_row` / `end_row` | 起止行 |
| `value_source` | 手动输入值或来源区域 |
| `manual_value` | 手动填充值 |
| `source_field` / `source_end_field` / `source_row` | 来源区域 |
| `start_row_mode` / `end_row_mode` | 行范围策略 |
| `reference_field` | 参考字段 |
| `overwrite_rule` | 覆盖规则 |

## 数据契约

节点先解析字段范围和行范围，再按覆盖策略填充目标区域。

必须限制最大目标单元格数量，避免一次配置造成过大输出。

## 执行模式

支持普通运行和预览运行。

支持取消：建议支持。

支持进度：建议支持，记录目标单元格数和实际写入数。

## 副作用与确认

副作用与外部资源说明：无外部副作用，只写本次运行输出。

是否需要用户确认：不需要，区域较大时建议确认或预览。

## 主程序交互边界

节点不依赖电子表格 UI。

所有区域配置通过 config 表达，主程序不理解区域填充算法。

## 运行记录

结果摘要建议包含目标区域、来源方式、目标单元格数、实际写入数。

失败提示应指出字段范围非法、行范围越界或超过上限。

## 验收方式

手动值填充区域正确。

只填空单元格策略正确。

超过单元格上限时拒绝并给出提示。

## 实现前置依赖

需要通用表格节点执行入口。

需要字段范围和行范围解析 helper。

## 简要模板补齐

节点名称：区域填充。

节点定位：数据处理 / 区域单元格填充。

优先级：P1。

当前状态：规划中，代码未实现。

要解决的问题：见上文“要解决的问题”章节。

用户看到的能力：见上文“用户看到的能力”描述。

第一版不解决的内容：见上文“不解决的内容”描述。

注册参数：
- node_type：FillRangeNode
- node_version：1.0
- plugin_id：core
- provider_type：builtin
- category：数据处理
- ui_visibility：visible
- enabled：规划期为 false；实现和验收后再按节点成熟度设为 true。
- display_name：区域填充
- config_schema：沿用本文“配置项草案”，后续落到统一 config_schema。
- input_ports：一个标准 TableRef 输入。
- output_ports：一个新的标准 TableRef 输出。
- implementation_ref：builtin.FillRangeNode（暂定内部执行入口，后续实现时绑定真实实现；不对普通 UI 暴露）。

输入说明：见上文“输入输出”章节；第一版按 input_ports 约束接收数据。

输出说明：见上文“输入输出”章节；输出必须使用标准引用或标准运行摘要。

配置说明：见上文“配置项草案”；配置只描述节点自身能力，不绑定具体 UI 控件。

数据流转方式：通过标准输入引用读取 TableRef，通过标准输出引用交付新的 TableRef。节点不直接读写主程序内部 Store，也不让主程序理解节点业务规则。

是否支持取消：支持；短耗时场景可在批处理边界检查取消，大表处理时按行或分批检查。

是否支持进度上报：支持低频进度上报，至少记录当前阶段、已处理数量和结果数量。

节点反馈：通过 NodeRun、RuntimeEvent 和结果摘要反馈开始、运行中、完成、失败、取消；摘要包含输入数量、输出数量、跳过数量和错误数量。

节点心跳：短耗时数据处理不强制主动心跳；处理大表或分批执行时按批次刷新 NodeRun.last_heartbeat。

心跳查询方式：通过 NodeRun.last_heartbeat、status、progress、current_stage、error 和 RuntimeEvent 查询节点活动状态。

后台极简模式：保留开始、结束、失败、取消和最终结果摘要；关闭逐行日志和高频 UI 刷新，大表场景保留低频进度和心跳。

外部资源与副作用：沿用本文“副作用与确认”章节；权限、审计、字段级追踪不写入默认节点方案。

性能影响等级：中。

主要性能消耗点：按行或按列扫描、复制或生成 TableRef。

失败场景：输入表不可读、字段不存在、配置不完整、数据类型不符合预期。

失败提示：提示缺失字段、非法配置项或失败阶段，并给出可修正的字段名、配置名和样例值。

验收方式：沿用本文“验收方式”章节，并补充校验运行记录、取消、进度、心跳和后台极简模式是否符合本节约束。
## 后续扩展
支持来源区域按横向或纵向重复填充。
