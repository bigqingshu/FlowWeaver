# 节点方案：MergeColumnsNode

更新时间：2026-07-05

## 基本信息

DataFlowKit 来源：`core.merge_columns` / 合并列

FlowWeaver 暂定类型名：`MergeColumnsNode`
...
把多个字段的值按分隔符组合为一个输出字段。

用户看到的能力：选择多个字段和分隔符，运行后得到合并结果字段。

不解决的内容：不做复杂模板渲染，不跨表合并，不写外部资源。
...
|---|---|
| `fields` | 参与合并的字段列表 |
| `separators` | 字段之间的分隔符列表 |
| `output_field` | 输出字段名 |
| `skip_empty` | 是否跳过空值 |
| `trim_value` | 是否清理值前后空白 |
| `empty_placeholder` | 空值占位 |
| `conflict_mode` | 输出字段冲突处理 |

## 数据契约

节点按行读取多个源字段，生成一个输出值。

第一版建议输出为新字段，冲突默认自动改名或报错，避免无意覆盖。
...
多字段选择和分隔符配置由 config 表达，执行层只读取 config。

不要求主程序识别合并列业务。

## 运行记录

结果摘要建议包含参与字段数、输出字段名、输出行数、空值处理策略。
...
空值处理符合 `skip_empty` 和 `empty_placeholder`。
...
- implementation_ref：builtin.MergeColumnsNode（暂定内部执行入口，后续实现时绑定真实实现；不对普通 UI 暴露）。
