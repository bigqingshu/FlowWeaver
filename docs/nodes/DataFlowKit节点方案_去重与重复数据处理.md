# 节点方案：DeduplicateRowsNode

更新时间：2026-07-05

## 基本信息

DataFlowKit 来源：`core.dedupe` / 去重 / 重复数据处理

FlowWeaver 暂定类型名：`DeduplicateRowsNode`
...
按字段组合或整行判断重复数据，输出去重结果或重复标记结果。

用户看到的能力：选择去重关键字段和保留策略，运行后得到去重后的表或带标记字段的表。

不解决的内容：不删除外部数据库记录，不做模糊相似去重。
...
输出行数可能减少，也可能保持不变并新增标记字段。

## 配置项草案

| 字段 | 含义 |
|---|---|
| `dedupe_mode` | 指定字段组合去重、整行完全重复 |
| `key_fields` | 去重关键字段 |
| `trim` | 比较前清理空白 |
| `ignore_case` | 是否忽略大小写 |
| `empty_key_policy` | 空键是否参与去重 |
| `keep_policy` | 保留第一条、保留最后一条、全部标记不删除 |
| `output_mode` | 输出去重后的数据或标记重复 |
| `add_marker_columns` | 是否添加标记字段 |

## 数据契约

节点按配置生成重复分组，并决定输出行。

行数变化必须进入结果摘要，避免用户误认为数据仍完整。
...
节点只产出新 `TableRef`，不让主程序理解去重策略。

不要求 WorkflowRunProcess 对行数变化做特殊处理。

## 运行记录

结果摘要建议包含关键字段、重复组数、移除行数、保留策略。
