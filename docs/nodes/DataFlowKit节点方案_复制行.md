...
DataFlowKit 来源：`core.copy_row` / 复制行

FlowWeaver 暂定类型名：`CopyRowsNode`
...
复制指定行若干次，并插入到表头、表尾或指定位置。

用户看到的能力：填写源行号、复制次数和插入位置，运行后得到行数增加的新表。

不解决的内容：不跨表复制行，不修改输入表，不写外部资源。
...
| `insert_row` | 插入参考行号 |
| `max_output_rows` | 可选，输出行数上限 |

## 数据契约

行号建议采用用户可理解的从 1 开始规则，并在错误提示中说明。
...
副作用与外部资源说明：无外部副作用，只写本次运行输出；属于行数增加节点。

是否需要用户确认：不需要，复制次数较大时建议确认或预览。

## 主程序交互边界

节点不要求主程序支持可变行数特殊分支。

输出表仍通过标准 `output_refs` 传递。

## 运行记录

结果摘要建议包含源行号、复制次数、插入位置、输出行数。
...
- implementation_ref：builtin.CopyRowsNode（暂定内部执行入口，后续实现时绑定真实实现；不对普通 UI 暴露）。
