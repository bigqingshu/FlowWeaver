...
> 当前边界：只做日志、事件、进度、诊断上下文、限流和脱敏；不进入节点业务 `config`；第一版不要求节点读取配置字
> 不适用范围：权限审计、数据产物保存、节点业务参数、插件私有配置、完整动态 schema 编辑器

## 1. 执行目标

配置字作为工作流运行反馈控制层，目标是降低后台工作流的无效反馈量，同时保留必要的诊断能力。
...
| 诊断保留清理任务 | 先只记录 `ttl_seconds` 建议，后续统一生命周期处理 |
