...
> 当前边界：只分析和规划后端；前端 UI 正在重构，本文不安排 UI 代码改动
...
| 问题 | 建议落点 |
| --- | --- |
| `output_slot_bindings` 已保存，但输入解析和表目录还没有完全消费 | 拆 `table_input_resolver` 或表目录 helper 时处理 |
| 写入类节点有第二张产物表，但槽位绑定只标 `status` | 拆 `table_write_nodes.py` 后集中补测试 |
| `SaveRunTableNode` 缺少 `output_table_slots` 定义 | 拆 default registry 时补齐 |

这些问题属于契约补强，不属于单纯解耦。建议每个点单独提交，避免“移动代码”和“改行为”混在一起。
