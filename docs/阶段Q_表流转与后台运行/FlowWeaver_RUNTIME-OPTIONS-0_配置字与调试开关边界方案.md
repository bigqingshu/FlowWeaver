...
本文是配置字方向的原始边界文档，设计边界保持有效，文中的“当前事实”“当前缺口”和推荐阶段应按立项时历史基线阅读。

截至 2026-07-11，以下方向已经落实：

- 工作流修订可保存固定 `runtime_options`，Avalonia 有独立修订配置窗口。
- 后端可解析 `system defaults -> profile -> workflow -> node override`。
- `NodeTaskModel` 使用独立的固定反馈策略字段，不污染节点业务 `config`。
...
不建议第一版加入：权限、审计、细粒度安全策略、外部存储清理策略、业务输出表自动清理策略、内存表预览快照生成策略。这些会把当前配置字方案拖回高耦合设计，或者让配置字从“反馈过滤”变成“数据产物生成”。
...
| 内存表预览快照 | 第一版不由配置字控制；快照会创建新的 runtime sqlite 数据产物，应归节点业务配置或诊断运行入口 |
