...
- 自行增加或重命名 API 字段。
- 自行定义后端状态含义。
- 因界面需要而改变 workflow、run、revision 或 capabilities 语义。
...
后端领域错误码、Revision冲突码和取消失败码由Codex继续补充到 `FlowWeaver_UI_BackendFactsContract.md`。Gemini不得根据错误Message文本反推业务状态，必须优先使用稳定错误码。
...
- 不改 Python EngineHost API 路径、字段名、capabilities 语义和 workflow 执行行为。
...
- 加载、校验和保存定义。
- 刷新Run、NodeRun。
- 刷新日志、TableRef和SharedPublication。
...
| `SUCCEEDED` | 已成功 | 禁用取消；可查节点、事件、数据和共享发布 |
| `FAILED` | 已失败 | 禁用取消；可查节点、事件和数据摘要 |
| `CANCELLED` | 已取消 | 禁用取消；可查节点和事件 |
...
| 状态 | UI 解释 | 建议动作 |
| --- | --- | --- |
| `PENDING`、`WAITING_DEPENDENCY` | 等待调度或依赖 | 只读展示 |
...
这些按钮需要后端表内容接口和读取能力 UI 进一步确认。当前最多可以在行内显示“可读取/不可读取”的派生文本，不打开新页面。
...
Codex不得为了方便UI而返回按钮定义，只能补充稳定事实、状态、能力和错误码。
