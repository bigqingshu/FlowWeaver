...
> 审核状态（2026-07-05）：部分已实现 / 分工文档仍有效
> 审核范围：本复制件用于 MD 状态审核，原始 docs/ 文件未修改；顺序按原文件创建时间从旧到新。
> 已实现：MainWindow 已收敛为 ShellHeaderView + AppShellPageHost，五个页面与组件目录已形成，语言/主题/绑定复核主线也已落地。
> 未实现：MainWindowViewModel 仍是集中式大 ViewModel，未拆成多个 partial 或页面 ViewModel；外部模块化也未实现。
> 原因：当前先保留业务状态集中，降低 UI 重构风险；外部模块、签名、安装器和热更新属于明确后置范围。
