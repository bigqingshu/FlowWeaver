...
> 审核状态（2026-07-05）：部分已实现 / 默认并行未启用
> 审核范围：本复制件用于 MD 状态审核，原始 docs/ 文件未修改；顺序按原文件创建时间从旧到新。
> 已实现：threaded 执行模式、max_concurrent_node_tasks、关闭/取消/异常/多 READY 并发验收和显式配置链路已经落地。
> 未实现：threaded 没有成为默认模式，默认仍保持 immediate 与 max_concurrent_node_tasks=1。
> 原因：这是文档中明确的保守边界，先允许显式启用和验证，再评估是否改默认。
