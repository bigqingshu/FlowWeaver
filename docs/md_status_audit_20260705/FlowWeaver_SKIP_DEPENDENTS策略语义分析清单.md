...
> 审核状态（2026-07-05）：分叉保留 / 独立策略未实现
> 审核范围：本复制件用于 MD 状态审核，原始 docs/ 文件未修改；顺序按原文件创建时间从旧到新。
> 已实现：SKIP_DEPENDENTS 枚举保留，显式配置会被校验拒绝，并有测试覆盖 reserved / unsupported 行为。
> 未实现：完整 SKIP_DEPENDENTS 运行时策略未实现。
> 原因：当前 DAG 是硬依赖模型，直接实现会与 CONTINUE_INDEPENDENT 高度重合或产生未定义跳过范围，因此先拒绝而不是伪装可用。
