...
> 已实现：最小桌面 UI、EngineHost health、API Client、工作流列表/运行入口、REST 恢复视图、WebSocket、日志/审计、数据摘要和验收已经落地。
> 未实现：K 阶段之外的结构化编辑、发布和产品化能力在后续阶段单独实现或后置。
> 原因：K 的目标是最小桌面闭环，不承担后续所有 UI 能力。
