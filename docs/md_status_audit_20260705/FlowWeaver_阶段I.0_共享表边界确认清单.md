...
> 已实现：共享表边界、SharedPublication、InputSnapshot、ReadLease、SharedTableReader、共享表节点和生命周期释放已在后续 I 阶段主线落地。
> 未实现：FOLLOW_LINEAGE 等高级共享表策略未实现。
> 原因：当前阶段只支持 LATEST / EXACT_VERSION，其他策略保留为后续扩展。
