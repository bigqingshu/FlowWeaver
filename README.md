...
- 下一步建议进入 P.1：发布归档脚本方案，先确认 `tools/create_portable_archive.py` 的参数、输入输出和安全检查，再决定是否实现最小 zip 生成
