...
- 下一步建议进入 P.2：runtime audit 与归档前检查，先实现可单测的 audit helper，不生成 zip、不清理 runtime，再由 P.3 接入最小归档脚本
