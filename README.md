...
- 下一步建议进入 N.3 便携版双进程发布布局设计：先设计目录结构、启动脚本边界和发布目录 smoke，不直接创建安装器
