...
- 下一步建议进入 O.10 真实 Desktop 最小 smoke：只做进程级启动、health、日志、退出清理和 token 脱敏验收，不做窗口交互或 UI 自动化
