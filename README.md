...
- 下一步建议进入 M.6 保存 revision 与冲突保护前置/实现：先补后端 `base_revision_id` 或 `expected_version` 检查，再接 Avalonia Save，继续不进入自由画布
