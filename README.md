...
- 下一步建议进入 O.6 Desktop 启动前置复核：先确认默认是否启动 Desktop、Desktop 失败/退出时 EngineHost 生命周期策略和最小自动化 smoke 边界，再决定是否真正接入 Desktop 自动启动
