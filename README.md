...
- 下一步建议进入 O.11 Desktop 生命周期真实路径收口：补充真实路径启动失败、用户中断和 keep EngineHost 策略验收，不扩展业务 UI 功能
