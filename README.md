...
- 下一步建议进入 O.9 独立 Desktop 启动入口生成：只生成 `start_flowweaver_desktop.cmd`、更新便携 README 和 N.4 文件级 smoke，不启动真实 Desktop
