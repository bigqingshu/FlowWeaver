...
- 下一步建议进入 P.3：最小归档脚本，调用 runtime audit 后生成 zip、`release-manifest.json`、`licenses/` 和外部 `.sha256`，仍不进入安装器、签名或上传
