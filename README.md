...
- 下一步建议进入 O.8 Desktop 入口策略与真实 Desktop smoke 前置决策：优先分析是否新增 `start_flowweaver_desktop.cmd`，再决定是否允许自动化启动真实 `Avalonia_UI.exe`
