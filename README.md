...
- UI 会持久化上次成功连接的 BaseUrl 和本机 token，启动后自动复用；点击“连接事件”后会记录事件流自动连接偏好，点击“停止”后取消该偏好；完整 WebSocket URL 必须脱敏
...
桌面端启动后会自动读取上次成功连接的 EngineHost `Base URL` 和本机 token，并尝试连接；无法连接时会在右上角状态区显示失败。首次使用或 token 轮换后，在主窗口输入当前 Base URL 和本机 token，点击 `Check` 保存并更新连接配置；点击“连接事件”会保存 token 并记录下次启动自动连接事件流，点击“停止”会取消该自动连接偏好。业务 API 和 RuntimeEvent WebSocket 仍需要 token。
