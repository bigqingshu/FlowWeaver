...
- 下一步建议进入 P.5：便携版用户手册骨架，先建立手册文件和章节结构，明确启动方式、token、日志、备份、关闭 Desktop 对运行中 workflow 的影响和当前不支持能力
