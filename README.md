...
- 下一步建议进入 O.3 launcher 真实启动层：补端口检查、EngineHost 启动、health 轮询、token 等待、日志写入和进程清理，但仍只走 `--no-desktop` 自动化路径、不接入生成器
