...
- 下一步建议进入 O.4 `--no-desktop` launcher 端到端 smoke：使用临时便携目录手工复制 launcher，验证 health、token、日志、脱敏和进程清理，不启动真实 Desktop
