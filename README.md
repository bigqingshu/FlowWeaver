...
- 下一步建议进入 O.2 launcher 文件边界：先新增 `tools/portable_launcher.py` 的接口草案和可单测 helper，不接入生成器、不启动真实 Desktop
